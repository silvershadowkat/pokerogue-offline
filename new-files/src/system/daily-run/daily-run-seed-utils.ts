...
/** Generate from already-selected calendar components without any timezone conversion. */
export function createOfflineDailySeedForDate(dateKey: string): string {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dateKey)) {
    throw new Error(`Invalid Offline Daily calendar date: ${dateKey}`);
  }
  return canonicalSeedFromText(`${OFFLINE_DAILY_ALGORITHM_VERSION}|${dateKey}`);
}

export function createCustomTextSeed(text: string): {
